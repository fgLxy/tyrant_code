...
public class WechatMiniLoginController {
	 ...
	 */
	@GetMapping("/login")
	public Map<String, Object> login(String code) throws SQLException {
		...
		try {
			...
			return ResponseUtils.response(GlobalValue.EXCEPTION_STATUS,
					e.getMessage());
		}

		String unionId = session.getUnionid();
		AuthInfo auth = authService.auth(session.getOpenid(),
				session.getSessionKey(), unionId);
	...
	public Map<String, Object> info(String signature, String rawData,
		...
		// 用户信息校验
		if (!this.wxService.getUserService().checkUserInfo(
				auth.getSessionKey(), rawData, signature)) {
			return ResponseUtils.response(RetCodeConstants.USER_INFO_ERROR,
					null);
		}
		// 解密用户信息
		WxMaUserInfo userInfo = null;
		try {
			userInfo = this.wxService.getUserService().getUserInfo(
					auth.getSessionKey(), encryptedData, iv);
		} catch (Throwable e) {
		...
		if (userInfo != null && !StringUtils.isEmpty(userInfo.getUnionId())) {
			...
			if (auth.getAuthFlag()) {
				...
				log.info("====>鉴权失败,unionId在系统中没有关联用户.UserInfo:{}",
	 ...
	 */
	@GetMapping("/phone")
	public Map<String, Object> phone(String signature, String rawData,
			String encryptedData, String iv) throws JsonProcessingException,
			SQLException {
		AuthInfo auth = authService.getAuthInfo();
		log.info(
				"=====>phone接口开始.sessionKey:{},signature:{}.rawData:{},encryptedData:{},iv:{}",
				auth.getSessionKey(), signature, rawData, encryptedData, iv);

		// 解密
		WxMaPhoneNumberInfo phoneNoInfo = null;
		try {
			phoneNoInfo = this.wxService.getUserService().getPhoneNoInfo(
					auth.getSessionKey(), encryptedData, iv);
		} catch (Throwable e) {
		...
		if (phoneNoInfo != null
				&& !StringUtils.isEmpty(phoneNoInfo.getPurePhoneNumber())) {
			auth = authService.linkByPhoneNumber(auth,
					phoneNoInfo.getPurePhoneNumber());
			if (auth.getAuthFlag()) {
				log.info("=======>鉴权成功.AuthInfo:{}", JSONUtils.toJson(auth));
				return ResponseUtils.response(GlobalValue.SUCCESS, auth);
			} else {
				log.info("====>鉴权失败,手机号不是系统中注册用户.phoneNumber:{}",
						phoneNoInfo.getPurePhoneNumber());
